function player_state_normal(){
	...
	if(abs(ground_speed) > 0 && abs(ground_speed) <= 6)
	{
		...
		if(ground && !skid)
		{
			animation_set_duration(animator, (max(0, 6-abs(obj_player.ground_speed))));
		}
	}
	...
	if(abs(ground_speed) >= 6)
	{
		...
		if(ground && !skid)
		{
			switch(character)
			{
				...
				default: 
					animation_set_duration(animator, (max(0, 7-abs(obj_player.ground_speed)))); 
				break;
			}
		}
	}
	...
	if(press_action && ground && !touching_ceiling && !force_roll)
	{
		...
		jump_anim_speed = floor(max(0, 1-abs(ground_speed)));
